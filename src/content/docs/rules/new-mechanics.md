...
The strength of the Dawn Gods rises and sets with the sun. Monsters that draw power from the Dusk Gods grow in strength during the night and weaken during the day. This has significant impact on a monster's stat block and the powers available to them.
...
- Use `WIS` for Blinded
- Use `WIS` for Dazed
- Use `CON` for Deafened
- Use `CHA` for Dominated
- Use `CON` for Helpless
- Use `STR` for Immobilized
- Use `WIS` for Marked
- Use `CON` for Petrified
- Use `STR` for Prone
- Use `STR` for Restrained
- Use `STR` for Slowed
- Use `WIS` for Stunned
- Use `WIS` for Surprised
- Use `CON` for Weakened

## Conditions

This is a list of the new conditions I have added to the game.
